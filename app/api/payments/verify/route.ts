 ...
 *
 * Flow:
 * 1. User sends USDC on Base to the treasury wallet
 * 2. Frontend posts the tx hash to this endpoint
 * 3. Worker verifies the payment on-chain via Base RPC
 * 4. If verified, grants credits and/or upgrades to Pro
 */
...
export async function POST(request: Request) {
  ...
  const verification = await verifyUsdcPayment(body.txHash, treasuryAddress, rpcUrl, {
    minimumAmount: type === "subscription"
      ? BigInt(PRO_PRICE_CENTS) * 10_000n // $5.00 in USDC (6 decimals)
      : 1_000_000n, // $1.00 minimum for top-ups
  });
  ...
  // Check if this tx was already processed (idempotency)
  const existing = await db
    .prepare("SELECT id FROM crypto_topups WHERE tx_hash = ?")
    .bind(body.txHash)
    .first();

  if (existing) {
    return Response.json({ ok: true, message: "Payment already processed", duplicate: true });
  }
  ...
  if (type === "subscription" && amountUsdCents >= PRO_PRICE_CENTS) {
    ...
    await db
      .prepare(
        "UPDATE user_subscriptions SET plan = 'pro', credits_balance = credits_balance + ?, credits_monthly = 2000, current_period_start = ?, current_period_end = ?, updated_at = ? WHERE user_id = ?",
      )
      .bind(creditsGranted, now, now + 30 * 24 * 3600, now, userId)
      .run();
  } else {
    // Top-up: just add credits
  ...
  await db
    .prepare(
      "INSERT INTO crypto_topups (id, user_id, type, chain, tx_hash, amount_usd_cents, credits_granted, status, completed_at, created_at) VALUES (?, ?, ?, 'base', ?, ?, ?, 'completed', ?, ?)",
    )
    .bind(
      crypto.randomUUID(),
      userId,
...
export async function GET(request: Request) {
  ...
  const payments = await db
    .prepare("SELECT * FROM crypto_topups WHERE user_id = ? ORDER BY created_at DESC LIMIT 10")
    .bind(userId)
    .all();
  ...
  return Response.json({
    ...
    proPriceUsdCents: PRO_PRICE_CENTS,
  });
}