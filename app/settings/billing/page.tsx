  ...
  const handleVerify = async () => {
    ...
    try {
      const res = await fetch("/api/payments/verify", {
        ...
        body: JSON.stringify({ txHash: txHash.trim(), type: "subscription" }),
      });

      const data = await res.json() as { ok: boolean; error?: string; creditsGranted?: number; plan?: string };
      ...
      setSuccess(true);
      setTxHash("");
      // Reload to update credits display
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
            ...
            <div className="text-sm space-y-3">
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">Go to <Link href="/settings/wallet" className="text-primary hover:underline">Wallet Settings</Link> and connect MetaMask</p>
                </div>
              </div>
              <div className="flex gap-3">
                <span className="flex-shrink-0 w-6 h-6 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-xs font-bold">2</span>
                <div>
                  <p className="font-medium">Send USDC to our treasury</p>
                  <p className="text-muted-foreground">
                    Send <span className="text-foreground font-medium">${proPriceUsd} USDC</span> on Base to:
                  </p>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <p className="font-medium">Paste your transaction hash below</p>
                  <p className="text-muted-foreground">We verify on-chain and activate your Pro subscription</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              ...
              {success && (
                <p className="text-sm text-green-400">
                  Payment verified! Activating Pro subscription... Reloading...
                </p>
              )}
            </div>
          </div>
        ...
        <section>
          <div className="bg-muted/30 rounded-lg px-4 py-3 text-xs text-muted-foreground space-y-1">
            <p>USD{CREDITS_PER_DOLLAR / 100} USDC = 1 credit (after {(PLATFORM_SPREAD * 100).toFixed(0)}% platform spread)</p>
            <p>Payments are verified on Base (Ethereum L2) — gas fees are ~$0.01</p>
            <p>Your treasury wallet address is configured on the server. Contact support if you need help.</p>
          </div>
        </section>
      </div>
    </div>
